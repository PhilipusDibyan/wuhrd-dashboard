...
export function exportExcel(records) {
  ...
  sheet['!cols'] = [6, 24, 25, 20, 18, 28, 10, 24, 18, 16, 14, 18].map(width => ({ wch: width }));
...
export function exportPdf(records) {
  if (!records.length) return showToast('Tidak ada data yang dapat diekspor.', 'error');
  ...
  pdf.autoTable({
    startY: 27,
    head: [['No', 'Nama', 'Posisi', 'Cabang', 'WhatsApp', 'MBTI', 'Tanggal Melamar']],
    body: records.map((record, index) => [index + 1, record.name || '—', record.position || '—', record.branch || '—', record.whatsapp || '—', record.mbti || '—', formatDate(record.appliedAt)]),
